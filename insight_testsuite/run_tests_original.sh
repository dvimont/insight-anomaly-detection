...
function check_project_struct {
  ...
  find_file_or_dir_in_project ${PROJECT_PATH} pom.xml  # inserted for maven-based project
...
function setup_testing_input_output {
  ...
  cp -r ${PROJECT_PATH}/pom.xml ${TEST_OUTPUT_PATH}  # inserted for maven-based project
